function shallow_water_cyclone_demo()
    ...
    for it = 1:nSteps
        % -- Geopotential based on current h and topography --
        Phi = g * (h + b);

        % -- 9a) Continuity Equation (will be used after momentum update) --
        % (Note: In the symplectic scheme we update h using the new velocity fields.)
        ...
        % Note: the height relaxation term (relax_rate*(h-h0)) will be applied during h-update

        % --- Symplectic Euler update ---
        % 1. Update momentum (u and v) first using the old h
        u_new = u + dt .* dudt;
        v_new = v + dt .* dvdt;
