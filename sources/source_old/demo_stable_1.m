function shallow_water_cyclone_demo()
    ...
    %% === Initialize Variables for Temporal Smoothing of Arrows ===
    prev_uq = uq;
    prev_vq = vq;
    prev_wq = wq;
    ...
    for it = 1:nSteps
        ...
        % Polar filter (helpful near poles, but smaller window).
        polar_filter = abs(PHI) > deg2rad(60);
        if any(polar_filter(:))
            h_smooth = smoothdata(h_new, 'gaussian', 5);  % was 9
        ...
        % Mid-latitude smoother with smaller window and lower blend
        midlat_filter = abs(PHI) > deg2rad(30) & abs(PHI) <= deg2rad(60);
        if any(midlat_filter(:))
            h_smooth = smoothdata(h_new, 'gaussian', 3);  % was 5
        ...
        % [REMOVED: Global smoothing every 50 steps]
...
function [u, v, h] = balanced_cyclone_initial_condition(PHI, LAM, h0, delta_h, sigma, lat_c, lon_c, Omega, g)
    ...
    %   PHI, LAM : 2D arrays of lat/long in radians
    %   h0       : Background fluid depth
    %   delta_h  : Max depression at cyclone center
    %   sigma    : Scale of the cyclone (radial extent)
    %   lat_c    : Cyclone center latitude (radians)
    %   lon_c    : Cyclone center longitude (radians)
    %   Omega    : Earth’s rotation rate
    %   g        : Gravitational acceleration
    %
    %   Returns: (u, v, h) arrays

    f_c = 2 * Omega * sin(lat_c);  % use f at center latitude
    [nLat, nLon] = size(PHI);
    ...
    for i = 1:nLat
        for j = 1:nLon
            ...
            % Fluid depth with a Gaussian depression:
            ...
            if r < 1e-6
                ...
                % Gradient wind balance V satisfies: V^2/r + f_c V = g dh/dr
                % => V^2 + (f_c r) V - g*r*(dhdr) = 0
                % Solve the quadratic for V
                dhdr = (2 * delta_h / sigma^2) * r * exp(-(r/sigma)^2);
                V = (-f_c * r + sqrt((f_c * r)^2 + 4*g*r*dhdr)) / 2;

                % Convert tangential velocity to (u,v)
                % The counterclockwise unit tangent is (-dlat/r, +dlon/r) in lat-lon coords
                u(i,j) = -V * (dlat / r);
                v(i,j) =  V * (dlon / r);
            end
        end
...
function [globe, lat_rad, lon_rad, x, y, z] = interactive_globe(texture_path)
    ...
    try
        ...
        % If the texture can't be loaded, just use a solid color.
        figure('Color', 'k');
        globe = surf(x, y, z, ...
                     'EdgeColor', 'none', ...
