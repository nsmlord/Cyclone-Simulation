clc; clear;

%% === Load Earth Globe with RGB Texture ===
[globe, lat_rad, lon_rad, x, y, z] = interactive_globe();

%% === Parameters ===
lat_res = 2; lon_res = 2;
R = 1; g = 9.81;
dx = deg2rad(lon_res) * R;
dy = deg2rad(lat_res) * R;
dt = 0.005;
nSteps = 40000;

%% === Topography (Gaussian mountain) ===
lat0 = deg2rad(30); lon0 = deg2rad(90);
b = 0.2 * exp(-((lat_rad - lat0).^2 + (lon_rad - lon0).^2) / 0.05);

%% === Initial Conditions ===
h0 = 1.0;
h = h0 * ones(size(lat_rad));
u = 0.05 * ones(size(lat_rad));  % small zonal wind
v = zeros(size(lat_rad));

%% === Coriolis Force ===
Omega = 7.2921e-5;
f = -2 * Omega * sin(lat_rad) * 10000;

%% === Local Unit Vectors ===
ex = -sin(lon_rad); ey = cos(lon_rad); ez = zeros(size(lat_rad));
nx = -cos(lon_rad).*sin(lat_rad); ny = -sin(lon_rad).*sin(lat_rad); nz = cos(lat_rad);

%% === Derivative Operators ===
d_dx = @(A) (circshift(A, [0, -1]) - circshift(A, [0, 1])) / (2*dx);
d_dy = @(A) (circshift(A, [-1, 0]) - circshift(A, [1, 0])) / (2*dy);

%% === Downsample for Quiver Plot ===
step = 15;
r = 1:step:size(lat_rad, 1);
c = 1:step:size(lat_rad, 2);
xq = x(r, c); yq = y(r, c); zq = z(r, c);
px = xq; py = yq; pz = zq;

wx = u .* ex + v .* nx;
wy = u .* ey + v .* ny;
wz = u .* ez + v .* nz;
uq = wx(r, c); vq = wy(r, c); wq = wz(r, c);

hold on;
h_arrows = quiver3(px, py, pz, uq, vq, wq, 0.5, 'w', 'LineWidth', 1.2);

%% === Constants ===
skipFrame = 4;
gamma = 0.01;       % friction
relax_rate = 0.01;  % fluid height relaxation
sigma_lat = deg2rad(10);
sigma_lon = deg2rad(10);

%% === Simulation Loop ===
for t = 1:nSteps
    % --- Pressure Gradient ---
    pressure = g * (h + b);
    dp_dx = d_dx(pressure);
    dp_dy = d_dy(pressure);

    % --- Advection ---
    u_adv = u .* d_dx(u) + v .* d_dy(u);
    v_adv = u .* d_dx(v) + v .* d_dy(v);

    % --- Momentum Update (Symplectic Euler) ---
    u_new = u + dt * (-u_adv - dp_dx + f .* v);
    v_new = v + dt * (-v_adv - dp_dy - f .* u_new);

    % --- Friction ---
    u_new = u_new - dt * gamma * u_new;
    v_new = v_new - dt * gamma * v_new;

    % --- Continuity ---
    div = d_dx(h .* u_new) + d_dy(h .* v_new);
    h_new = h - dt * div;

    % === 🌞 Solar Heating ===
    solar_speed = 2 * pi / 100;  % full equatorial loop in 100 frames
    solar_lon = mod(solar_speed * t, 2*pi);
    solar_lat = deg2rad(23.5) * cos(2 * pi * t / 365);  % seasonal migration

    Q = 0.005 * exp(-((lat_rad - solar_lat).^2 / sigma_lat^2 + ...
                      (lon_rad - solar_lon).^2 / sigma_lon^2));
    h_new = h_new + dt * Q;

    % --- Height Relaxation ---
    h_new = h_new + dt * relax_rate * (h0 - h_new);

    % --- Update Fields ---
    u = u_new;
    v = v_new;
    h = h_new;

    % --- Project Wind to 3D ---
    wx = u .* ex + v .* nx;
    wy = u .* ey + v .* ny;
    wz = u .* ez + v .* nz;
    uq = wx(r, c); vq = wy(r, c); wq = wz(r, c);

    % --- Move Arrow Anchors Along Flow ---
    px = px + 0.002 * uq;
    py = py + 0.002 * vq;
    pz = pz + 0.002 * wq;
    mag = sqrt(px.^2 + py.^2 + pz.^2);
    px = px ./ mag; py = py ./ mag; pz = pz ./ mag;

    % --- Update Plot ---
    if mod(t, skipFrame) == 0
        set(h_arrows, 'XData', px, 'YData', py, 'ZData', pz, ...
                      'UData', uq, 'VData', vq, 'WData', wq);
        title(sprintf('Solar Heating Simulation'), ...
              'Color', 'w', 'FontSize', 14);
        drawnow limitrate;
    end
end
