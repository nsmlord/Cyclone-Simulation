...
% Gaussian bump to simulate mountain near lat = 30°, lon = 90°
lat0 = deg2rad(30);
lon0 = deg2rad(90);
b = 0.2 * exp(-((lat_rad - lat0).^2 + (lon_rad - deg2rad(lon0)).^2) / 0.05);  % topography

%% === Initial Conditions ===
h0 = 1.0;
h = h0 * ones(size(lat_rad));  % fluid height above terrain
u = 0.1 * ones(size(lat_rad));  % eastward wind
v = zeros(size(lat_rad));       % no northward wind

%% === Coriolis Force ===
Omega = 7.2921e-5;
f = -2 * Omega * sin(lat_rad) * 10000;  % scaled for visibility
...
%% === Simulation Loop ===
skipFrame = 4;  % update display every N frames
for t = 1:nSteps
    % Derivatives of free surface height (h + b)
    eta = h + b;
    eta_x = d_dx(eta);
    eta_y = d_dy(eta);

    % Advection terms (optional: simple center)
