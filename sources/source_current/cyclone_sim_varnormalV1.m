function shallow_water_cyclone_demo_with_diagnostics()
    ...
    % diagnostics for total mass, energy, potential enstrophy, angular momentum,
    % and maximum wind speed. It can run in validation mode (forcing disabled)
    % or study mode (forcing enabled) to explore cyclone evolution under different forcing.
    
    clc; clear;

    %% USER OPTIONS: Choose study mode and forcing parameters
    % Set study_mode = true to run the full forcing simulation and vary parameters.
    study_mode = true;  
    % In study mode, forcing is enabled and validation mode is disabled.
    params.validation_mode = ~study_mode;  
    % For sensitivity analysis, you can modify these parameters:
    ...
    % Time parameters
    dt       = 0.0001;    % time step (nondimensional)
    nSteps   = 50000;        % number of simulation steps (make smaller for quick tests)
    ...
    % In validation mode, turn off topography: 
    ...
    h0 = params.h0;  % use background depth from params
    ...
    %% === 8) Model Constants ===
    % Use parameters from the params structure:
    if params.validation_mode
        gamma      = 0;    % no friction
        relax_rate = 0;    % no height relaxation
    else
        gamma      = params.gamma;
    ...
    % We'll store total mass, total energy, enstrophy, momentum, and max wind speed each step.
    ...
    for it = 1:nSteps
        ...
        % Damping factor (validation mode vs study mode)
        if params.validation_mode
            damp_factor = 1.0;
        else
            damp_factor = 0.8;
        ...
        % Apply friction (disabled in validation mode)
        dudt = dudt - gamma .* u;
        dvdt = dvdt - gamma .* v;
        ...
        if params.validation_mode
            ...
            Q_forcing = params.Q_amp * exp(-(((PHI - solar_lat).^2)/deg2rad(15)^2 + ((LAM - solar_lon).^2)/deg2rad(15)^2));
        end
        ...
        % Smoothing (applied only if NOT in validation mode)
        if ~params.validation_mode
        ...
        diag_max_speed(it) = max(sqrt(u.^2 + v.^2), [], 'all');  % Maximum wind speed
        ...
        if mod(it, skipFrame) == 0
            ...
            title(sprintf('Shallow-Water Cyclone Demo'), ...
                  'Color', 'w', 'FontSize', 14);
            drawnow limitrate;
    ...
    xlabel('Time (nondim)'); ylabel('Max Wind Speed');
    title('Max Wind Speed vs. Time');
    grid on;

    sgtitle('Diagnostics: Mass, Energy, Enstrophy, Momentum, and Max Wind Speed','FontSize',14);
end
